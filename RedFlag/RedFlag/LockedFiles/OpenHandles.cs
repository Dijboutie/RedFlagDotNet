...
namespace RedFlag
{
    class OpenHandles
    {
        ...
        static public string GetProcessesUsingFile(string FileName)
        {
            ...
            foreach (Process p in processes)
            {
                ...
                if (m_ipProcessHwnd == null) continue;
                    ...
                    {
                        string strTemp = "";
                        strTemp = GetFileDetails(handles[nDex]).Name;
                        if (strTemp != "")
                        {
                            if (strTemp.ToUpper() == FileName.ToUpper())
                            {
        ...
        public static FileDetails GetFileDetails(Win32API.SYSTEM_HANDLE_INFORMATION sYSTEM_HANDLE_INFORMATION)
        {
            ...
            if (!Win32API.DuplicateHandle(Win32API.GetCurrentProcess(), sYSTEM_HANDLE_INFORMATION.Handle, Win32API.GetCurrentProcess(), out ipHandle, 0, false, Win32API.DUPLICATE_SAME_ACCESS)) return fd;

            ipBasic = Marshal.AllocHGlobal(Marshal.SizeOf(objBasic));
            if (Win32API.NtQueryObject(ipHandle, (int)Win32API.ObjectInformationClass.ObjectBasicInformation, ipBasic, Marshal.SizeOf(objBasic), ref nLength)!=0) return fd;
            ...
            // Debug.Print("GrantedAccess Value: 0X" + objBasic.GrantedAccess.ToString("X"));
            // Debug.Print("Attributes value: 0X"+objBasic.Attributes.ToString("X"));
            if (objBasic.GrantedAccess ==ACCESS_MASK_PIPE_SIGNATURE)
            {               
                return fd; // do not query pipes!!
            }
        ...
        private static string GetObjectNameInfo(IntPtr ipHandle)
        {
            ...
            if (nLength > 0)
            {
                ...
                if (nReturn == 0)
                {
                    ...
                    //strObjectName = Marshal.PtrToStringUni(objObjectName.Name.Buffer, objObjectName.Name.Length >> 1);
                    //if (strObjectName != null) fd.Name = GetRegularFileNameFromDevice(strObjectName);
                }
                else
                {
