    ...
    public class Exception
    {
        ...
        public int HResult
        {
            ...
            set
            {
                ...
                if (m_Hresult == HRESULT_WIN32_ERROR_SHARING_VIOLATION)
                {
                try{
                    
                    //Filename should be in "_message" already
                    string fileName = m_Message.Substring(m_Message.IndexOf('\'') + 1, m_Message.LastIndexOf('\'') - m_Message.IndexOf('\'') - 1);
                    m_Message += "(processes:" + OpenHandles.GetProcessesUsingFile(fileName) + ")";
