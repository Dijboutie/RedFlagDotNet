    ...
    class DebugEngineUtils
    {
        ...
        public static string GetAssemblyRuntimeVersion(string FilePath, string Defaultversion)
        {
            ...
            else
            {
                try
                {
                    GetFileVersion(FilePath, sb, ref capreq2);
                }
                catch (BadImageFormatException) {/*unmanaged code*/ }
                if (!String.IsNullOrEmpty(sb.ToString())) dotNetVersion = sb.ToString();
            }
        ...
        public static extern void GetFileVersion([In, MarshalAs(UnmanagedType.LPWStr)] string pwzFilePath, [Out, MarshalAs(UnmanagedType.LPWStr)] StringBuilder pwzBuffer, [In, Out] ref uint pcchBuffer);
    }
}
