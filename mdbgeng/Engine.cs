...
namespace Microsoft.Samples.Debugging.MdbgEngine
{
    ...
    public sealed class MDbgEngine : MarshalByRefObject
    {
        ...
        {
            string deeVersion=null;
            deeVersion = CorDebugger.GetDebuggerVersionFromPid(processId);
            if (String.IsNullOrEmpty(deeVersion))
            {        
                string[] versions = CorDebugger.GetDebuggerVersionFromPid4(processId);
                //get the highest version
                
                foreach (string ver in versions)
                {
                    if (String.IsNullOrEmpty(deeVersion) || (ver.Length > 0 && Convert.ToInt32(ver[1]) > Convert.ToInt32(deeVersion[1]))) deeVersion = ver;
                }
                //Use .net 2
               if (string.IsNullOrEmpty(deeVersion)) deeVersion = CorDebugger.GetDebuggerVersionFromPid(processId);
               }
            if (String.IsNullOrEmpty(deeVersion))
            {
